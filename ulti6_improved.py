    ...
    def train(self, episodes=500, render_interval=50, layer_min=None, layer_max=None):
        ...
        for ep in range(episodes):
            ...
            if total_reward > self.best_reward:
                ...
                if layer_min:
                    self.save_model(f'improved_best_{layer_min}_{layer_max}_path_model.pth')
                else:
                    self.save_model(f'improved_best_{"3d" if self.is_3d else "2d"}_path_model.pth')
...
def plot_comparison(obstacle_map, all_paths, metadata):
    ...
    plt.title('Path Planning Comparison (Improved DQN)', fontsize=14)
