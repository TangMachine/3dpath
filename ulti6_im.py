...
########################################################
# Improved Classes with Table-based State Space       #
########################################################
...
class ImprovedPathPlanningEnv:
    def __init__(self, obstacle_map, start, goal, is_3d=False, elevation_data=None, delta_z=5, max_z=150):
        ...
        self.combined_map_table = np.zeros((self.map_height, self.map_width), dtype=np.float32)

        # 设置目标位置在组合表格中（固定）
        # 编码: 0.0 = 空地, 1.0 = 已发现障碍物, 2.0 = 目标位置
        if self.is_3d:
            self.combined_map_table[goal[0], goal[1]] = 2.0
        else:
            self.combined_map_table[goal[0], goal[1]] = 2.0
    ...
    def reset(self):
        ...
        # 重置组合表格中的障碍物（保留目标位置）
        # 只重置非目标位置的格子
        mask = self.combined_map_table != 2.0
        self.combined_map_table[mask] = 0.0

        # 初始化时更新周围的障碍物
        if self.is_3d:
            self.update_discovered_obstacles(self.current_pos[0], self.current_pos[1])
        else:
            self.update_discovered_obstacles(self.current_pos[0], self.current_pos[1])
    ...
    def get_state(self):
        ...
        state = np.stack([
            ...
            self.combined_map_table
        ], axis=0)
        return state

    def update_discovered_obstacles(self, x, y):
        """
        更新组合表格中发现的障碍物
        检查周围8个方向的格子，如果是障碍物则标记为1.0
        目标位置保持为2.0不变
        """
        ...
        for dx, dy in directions:
            ...
            if (0 <= nx < self.map_height and 0 <= ny < self.map_width):
                if self.grid[nx, ny] == 1:  # 如果是障碍物
                    # 只有当前不是目标位置时才标记为障碍物
                    if self.combined_map_table[nx, ny] != 2.0:
                        self.combined_map_table[nx, ny] = 1.0

    def get_valid_actions(self):
        """
        根据当前位置和已发现的障碍物，返回有效的action列表
        """
        valid_actions = []
        ...
        if self.is_3d:
            ...
            for action_idx, (dx, dy, dz) in enumerate(self.actions_3d):
                ...
                # 检查已发现的障碍物
                if self.combined_map_table[nx, ny] == 1.0:
                    # 如果是3D环境，检查是否可以飞越障碍物
    ...
    def step(self, action):
        ...
        if 0 <= nx < self.grid.shape[0] and 0 <= ny < self.grid.shape[1]:
            # 更新发现的障碍物
            self.update_discovered_obstacles(nx, ny)
...
class ImprovedDQNAgent:
    ...
    def act(self, state):
        ...
        # 确保状态的形状正确 (1, 3, map_height, map_width)
    ...
    def visualize_state_tables(self, state=None):
        ...
        # 组合地图表（障碍物和目标）
        axes[1].imshow(state[1], cmap='viridis', origin='upper')
        axes[1].set_title('Combined Map Table (Obstacles + Goal)')
    ...
    def visualize_action_filtering(self, state=None):
        ...
        local_view = obstacle_view[row_start:row_end, col_start:col_end]

        ax.imshow(local_view, cmap='Reds', origin='upper')
...
class ImprovedLayeredDQNEnv(ImprovedPathPlanningEnv):
    ...
    def step(self, action):
        ...
        # 更新发现的障碍物
        self.update_discovered_obstacles(new_x, new_y)
