...
def create_obstacle_map(data, metadata):
    ...
    # padded_map = np.pad(obstacle_map, pad_width=1, mode='constant', constant_values=0)
    # for r in range(2, padded_map.shape[0] - 2):
    #     for c in range(2, padded_map.shape[1] - 2):
    #         if padded_map[r, c] == 1:
    #             # 将周围所有标记为0的格子标记为3
    #             neighbors = padded_map[r - 1:r + 2, c - 1:c + 2]
    #             obstacle_map[r - 2:r + 1, c - 2:c + 1] = np.where(neighbors == 0, 3,
    #                                                               obstacle_map[r - 2:r + 1, c - 2:c + 1])
...
class PathPlanningEnv:
    def __init__(self, obstacle_map, start, goal, is_3d=False, elevation_data=None, delta_z=5,max_z=150):
        ...
        self.max_z = max_z
        if self.is_3d:
            # 26 个方向: 所有 (dx, dy, dz) ∈ [-1,0,1]^3 \ {(0,0,0)}
            self.actions_3d = []
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    for dz in [-1, 0, 1]:
...
class DQN(nn.Module):
    def __init__(self, input_dim, output_dim, is_3d=False):
    ...
    #     return self.fc(x)
    #     self.fc = nn.Sequential(
    #         nn.Conv2d(1, 16, kernel_size=7, stride=2, padding=3),  # 输出尺寸: (1700-7+6)/2 +1 = 850
    #         nn.ReLU(inplace=True),
    #         nn.MaxPool2d(kernel_size=2, stride=2),  # 输出尺寸: 850/2 = 425
    #
    #         # 卷积块2: 16通道 → 32通道，尺寸425→212
    #         nn.Conv2d(16, 32, kernel_size=5, stride=1, padding=2),  # 保持尺寸425
    #         nn.ReLU(inplace=True),
    #         nn.MaxPool2d(kernel_size=2, stride=2),  # 输出尺寸: 425/2 = 212（向下取整）
    #
    #         # 卷积块3: 32通道 → 64通道，尺寸212→106
    #         nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # 保持尺寸212
    #         nn.ReLU(inplace=True),
    #         nn.MaxPool2d(kernel_size=2, stride=2),  # 输出尺寸: 212/2 = 106
    #
    #         # 卷积块4: 64通道 → 128通道，尺寸106→53
    #         nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # 保持尺寸106
    #         nn.ReLU(inplace=True),
    #         nn.MaxPool2d(kernel_size=2, stride=2),  # 输出尺寸: 106/2 = 53
    #     )
    #     if is_3d:
    #         # 三维环境网络结构
    #         self.classifier = nn.Sequential(
    #             nn.AdaptiveAvgPool2d((1, 1)),  # 输出128@1×1，兼容任意输入
    #             nn.Flatten(),  # 展平为128维向量
    #             nn.Linear(128, 64),  # 全连接层
    #             nn.ReLU(inplace=True),
    #             nn.Dropout(0.5),  # 防止过拟合
    #             nn.Linear(64, 26)  # 输出26分类
    #         )
    #
    #     else:
    #         # 二维环境网络结构
...
class DQNAgent:
    def __init__(self, env, is_3d=False):
        self.env = env
        self.is_3d = is_3d
        self.state_dim = 6 if is_3d else 4
        self.action_dim = 10 if is_3d else 8
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = DQN(self.state_dim, self.action_dim,is_3d).to(self.device)  # 将模型迁移到 GPU
        self.target_model = DQN(self.state_dim, self.action_dim,is_3d).to(self.device)  # 同样迁移目标模型
        ...
        for c in range(data.shape[1]):
            ...
            if val == nodata:
                ...
                # if building height < layer_min => passable
                    ...
                    layered_map[r, c] = 2
    # padded_map = np.pad(layered_map, pad_width=1, mode='constant', constant_values=0)
    # for r in range(2, padded_map.shape[0] - 2):
    #     for c in range(2, padded_map.shape[1] - 2):
    #         if padded_map[r, c] == 1:
    #                     # 将周围所有标记为0的格子标记为3
    #             neighbors = padded_map[r - 1:r + 2, c - 1:c + 2]
    #             layered_map[r - 2:r + 1, c - 2:c + 1] = np.where(neighbors == 0, 3,
    #                                                                       layered_map[r - 2:r + 1, c - 2:c + 1])
    return layered_map
...
class LayeredPathPlanningEnv(PathPlanningEnv):
    ...
    def step(self, action):
        ...
        if cell_val == 1:
            ...
            reward = -5
            self.current_pos = (new_x, new_y)# heavy penalty for "3" cells
            # passable
        else:
...
def plot_comparison(obstacle_map, all_paths, metadata):
    ...
    style_config = {
        ...
        ('Layered DQN 30-40m', True): {'color': 'orange', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 30-40m', False): {'color': 'orange', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 40-50m', True): {'color': 'green', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 40-50m', False): {'color': 'green', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 50-60m', True): {'color': 'purple', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 50-60m', False): {'color': 'purple', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 60-70m', True): {'color': 'brown', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 70-80m', True): {'color': 'pink', 'linestyle': '-', 'linewidth': 2},
        ('Layered DQN 80-90m', True): {'color': 'gray', 'linestyle': '-', 'linewidth': 2},
    }
