...
def astar_3d(data, metadata, start, end, delta_z=1):
    def heuristic(a, b):
...
class DQNAgent:
    ...
    def get_path(self, max_steps=100000):
        ...
        self.epsilon = 0.05 # 禁用探索，完全依赖策略网络
        try:
        # 尝试获取完整路径
            path = []
            state = self.env.reset()
...
if __name__ == "__main__":
    try:
        ...
        # 4) Create 3D path and run astar_3d
        print("3D路径规划中...")
        delta_z = 1
        start_3d = (start_row, start_col, data[start_row, start_col] + 5)
        end_3d = (end_row, end_col, data[end_row, end_col] + 5)
        path_3d = astar_3d(data, metadata, start_3d, end_3d, 5)

        # 5) Demonstration of standard DQN 2D and 3D
        dtrain = True
        if dtrain:
        ...
        # 6) Layered approach demonstration
        # Let's assume we define multiple layers (0-10m, 10-20m, 20-30m).
        # We'll plan on a single chosen layer, e.g. 10-20m
        layer_min, layer_max = 10, 20
        layered_map_10_20 = create_layered_obstacle_map(data, metadata, layer_min, layer_max)
        # A* on a single layer
        print(f"Layered A*: {layer_min}-{layer_max}m")
        layered_astar_path = layered_astar_2d(layered_map_10_20, (start_row, start_col), (end_row, end_col))
        layered_astar_result = {'path': layered_astar_path if layered_astar_path else [], 'success': True if layered_astar_path else False}

        # DQN on a single layer
        print(f"Layered DQN: {layer_min}-{layer_max}m")
        layered_env = LayeredDQNEnv(layered_map_10_20, (start_row, start_col), (end_row, end_col))
        layered_agent = DQNAgent(layered_env, is_3d=False)
        layered_agent.train(episodes=25)  # fewer episodes for demo
        layered_dqn_result = layered_agent.get_path()

        # Let’s also add a multi-layer approach demonstration:
        # For example, we pick multiple layers and plan on each layer. We'll keep it simple in code:
        layer_ranges = [(0, 10), (10, 20), (20, 30)]
        multi_layer_paths = []

        for (lmin, lmax) in layer_ranges:
            print(f"Testing A* on layer: {lmin}-{lmax}m")
            lm = create_layered_obstacle_map(data, metadata, lmin, lmax)
            path_ = layered_astar_2d(lm, (start_row, start_col), (end_row, end_col))
            if path_:
                multi_layer_paths.append((path_, (lmin, lmax)))

        # 7) Collect results for final plots
        all_paths = [
            ('A* 3D', {'path': path_3d, 'success': True}, True),
            ('DQN 3D', dqn_3d_result, True),
            ('Layered A*', layered_astar_result, False),
            ('Layered DQN', layered_dqn_result, False),
        ]

        # 8) Plot single-layer approach
        print("绘制指定高度层(10-20m)的路径规划结果...")
        plot_comparison(layered_map_10_20, [
            ('Layered A*', layered_astar_result, False),
            ('Layered DQN', layered_dqn_result, False)
        ], metadata)

        # 9) Plot multi-layer results in a single figure if we like
        # We'll pick one color for each layer; we can quickly do a combined figure:
        plt.figure(figsize=(14, 10))
        plt.imshow(obstacle_map, cmap='gray_r', origin='upper', alpha=0.5)
        colors = ['magenta', 'yellow', 'lightblue']
        for idx, (path_, (lmin, lmax)) in enumerate(multi_layer_paths):
            if len(path_) > 1:
                rows, cols = zip(*path_)
                plt.plot(cols, rows, label=f'Layer {lmin}-{lmax}m A*', color=colors[idx % len(colors)])
                plt.scatter(cols[-1], rows[-1], color=colors[idx % len(colors)], marker='X', edgecolors='black')
        plt.legend()
        plt.title('Multiple Layers A* Paths')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.grid(True, linestyle=':', alpha=0.5)
        plt.tight_layout()
        ...
        # 10) Plot overall comparison
        print("绘制所有方案对比图...")
        plot_comparison(obstacle_map, all_paths, metadata)

        # Example CSV export (optional):
        # table_2d = generate_table(path_2d, metadata, data, is_3d=False)
        # export_csv(table_2d, 'astar_2d_path.csv')
